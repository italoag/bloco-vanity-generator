...
2. Launch the TUI with default settings: `./bloco-vgen --prefix cafe --progress`.
   - Verify the ASCII banner animates smoothly without stalling progress updates.
3. Disable animation via env var: `BLOCO_DISABLE_ANIMATION=1 ./bloco-vgen --prefix cafe --progress`.
   - Confirm the banner renders statically with no animation artifacts.
4. Re-enable explicitly: `BLOCO_DISABLE_ANIMATION=1 ./bloco-vgen --prefix cafe --progress --enable-animation`.
