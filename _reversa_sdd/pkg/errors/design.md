# Módulo pkg/errors, Design Técnico
...
A unit `pkg/errors` é implementada no legado principalmente em `pkg/errors/types.go`. Sua interface é formada por funções, structs e contratos internos consumidos pelos demais módulos do monólito CLI. 🟢

| Símbolo | Assinatura / Entrada | Retorno | Observação |
|---------|----------------------|---------|------------|
| `WrapError` | parâmetros: err error, errorType ErrorType, operation string, message string | `*BlocoError` | 🟢 |
...
3. O algoritmo principal executa: Captura limitada de stack via runtime.Caller. 🟢
...
|---------|---------------------|-----------|
| A unit permanece encapsulada em `pkg/errors` e é consumida por contratos internos. | `pkg/errors/types.go` | 🟢 |
...
|---|---|---|---:|
| `BlocoError` | `Type: ErrorType` | obrigatório no contrato legado | 🟢 |
| `BlocoError` | `Operation: string` | obrigatório no contrato legado | 🟢 |
| `BlocoError` | `Message: string` | obrigatório no contrato legado | 🟢 |
| `BlocoError` | `Cause: error` | opcional no contrato legado | 🟢 |
...
- 🟡 Stack opcional deve evitar custo excessivo e vazamento de dados sensíveis.
