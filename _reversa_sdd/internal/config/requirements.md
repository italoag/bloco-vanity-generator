...
A unit `internal/config` centraliza defaults, variáveis BLOCO_* e validação de limites operacionais. A especificação é baseada nos artefatos Archaeologist/Detective/Architect e nos arquivos principais `internal/config/config.go`. 🟢

## Responsabilidades

- Manter o contrato operacional descrito para `internal/config` no legado. 🟢
...
|----|-----------|-----------|-------------------|
| RF-01 | Reimplementar a responsabilidade principal da unit `internal/config`. | Must | Chamadas equivalentes às funções mapeadas em `internal/config/config.go` produzem resultados compatíveis. 🟢 |
...
```gherkin
Dado uma entrada válida para `internal/config`
...
Dado que um módulo consumidor depende de `internal/config`
