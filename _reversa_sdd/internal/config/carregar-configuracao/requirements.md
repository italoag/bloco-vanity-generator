...
A unit `internal/config/carregar-configuracao` carrega defaults, aplica variáveis BLOCO_* e valida limites antes da CLI iniciar. A especificação é baseada nos artefatos Archaeologist/Detective/Architect e nos arquivos principais `internal/config/config.go`. 🟢

## Responsabilidades

- Manter o contrato operacional descrito para `internal/config/carregar-configuracao` no legado. 🟢
...
- Thread count deve estar entre 1 e 128. 🟢
- Quiet mode e verbose output são mutuamente exclusivos. 🟢
- KDF permitido: scrypt, pbkdf2, pbkdf2-sha256 ou pbkdf2-sha512. 🟢

## Requisitos Funcionais

| ID | Requisito | Prioridade | Critério de Aceite |
|----|-----------|-----------|-------------------|
| RF-01 | Reimplementar a responsabilidade principal da unit `internal/config/carregar-configuracao`. | Must | Chamadas equivalentes às funções mapeadas em `internal/config/config.go` produzem resultados compatíveis. 🟢 |
...
|------|--------------------|---------------------|-----------|
| Manutenibilidade | A unit deve preservar separação modular em `internal/config`. | `internal/config/config.go` | 🟢 |
...
| Operabilidade | Erros e estados devem permanecer rastreáveis por mensagens ou structs de domínio. | `internal/config/config.go` | 🟢 |
...
```gherkin
Dado uma entrada válida para `internal/config/carregar-configuracao`
...
Dado que um módulo consumidor depende de `internal/config/carregar-configuracao`
