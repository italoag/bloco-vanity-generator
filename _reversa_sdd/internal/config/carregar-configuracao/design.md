# Caso de Uso: Carregar Configuração, Design Técnico
...
A unit `internal/config/carregar-configuracao` é implementada no legado principalmente em `internal/config/config.go`. Sua interface é formada por funções, structs e contratos internos consumidos pelos demais módulos do monólito CLI. 🟢
