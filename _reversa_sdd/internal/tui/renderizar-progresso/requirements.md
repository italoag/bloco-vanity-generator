# Caso de Uso: Renderizar Progresso TUI
...
A unit `internal/tui/renderizar-progresso` renderiza progresso de geração e resultados via Bubble Tea. A especificação é baseada nos artefatos Archaeologist/Detective/Architect e nos arquivos principais `internal/tui/manager.go`, `internal/tui/progress.go`, `internal/tui/stats.go`, `internal/tui/benchmark.go`, `internal/tui/styles.go`, `internal/tui/logo.go`, `internal/tui/utils.go`. 🟢

## Responsabilidades

- Manter o contrato operacional descrito para `internal/tui/renderizar-progresso` no legado. 🟢
...
|----|-----------|-----------|-------------------|
| RF-01 | Reimplementar a responsabilidade principal da unit `internal/tui/renderizar-progresso`. | Must | Chamadas equivalentes às funções mapeadas em `internal/tui/manager.go`, `internal/tui/progress.go`, `internal/tui/stats.go`, `internal/tui/benchmark.go`, `internal/tui/styles.go`, `internal/tui/logo.go`, `internal/tui/utils.go` produzem resultados compatíveis. 🟢 |
...
```gherkin
Dado uma entrada válida para `internal/tui/renderizar-progresso`
...
Dado que um módulo consumidor depende de `internal/tui/renderizar-progresso`
