...
A unit `internal/crypto/gerar-keystore-v3` cifra private key em estrutura KeyStore V3 com KDF, AES-CTR e MAC. A especificação é baseada nos artefatos Archaeologist/Detective/Architect e nos arquivos principais `internal/crypto/ethereum.go`, `internal/crypto/bitcoin.go`, `internal/crypto/solana.go`, `internal/crypto/checksum.go`, `internal/crypto/keystore.go`, `internal/crypto/password.go`, `internal/crypto/pools.go`, `internal/crypto/random.go`, `internal/crypto/validation.go`, `internal/crypto/generator.go`. 🟢

## Responsabilidades

- Manter o contrato operacional descrito para `internal/crypto/gerar-keystore-v3` no legado. 🟢
...
- Por decisão humana, `EncryptPrivateKeyWithKDF()` é detalhe interno; consumidores devem usar `GenerateKeyStore()`/serviço de alto nível. 🟢
- Por decisão humana, persistência Solana deve ser criptografada/segura e não salvar `.key` bruto. 🟢

## Requisitos Funcionais

| ID | Requisito | Prioridade | Critério de Aceite |
|----|-----------|-----------|-------------------|
| RF-01 | Reimplementar a responsabilidade principal da unit `internal/crypto/gerar-keystore-v3`. | Must | Chamadas equivalentes às funções mapeadas em `internal/crypto/ethereum.go`, `internal/crypto/bitcoin.go`, `internal/crypto/solana.go`, `internal/crypto/checksum.go`, `internal/crypto/keystore.go`, `internal/crypto/password.go`, `internal/crypto/pools.go`, `internal/crypto/random.go`, `internal/crypto/validation.go`, `internal/crypto/generator.go` produzem resultados compatíveis. 🟢 |
...
```gherkin
Dado uma entrada válida para `internal/crypto/gerar-keystore-v3`
...
Dado que um módulo consumidor depende de `internal/crypto/gerar-keystore-v3`
...
| `internal/crypto/keystore.go` | `EncryptPrivateKeyWithKDF` | 🟢 interno; não contrato público |
