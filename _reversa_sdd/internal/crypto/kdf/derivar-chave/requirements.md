...
A unit `internal/crypto/kdf/derivar-chave` normaliza KDF, valida parâmetros e deriva chave com scrypt/PBKDF2. A especificação é baseada nos artefatos Archaeologist/Detective/Architect e nos arquivos principais `internal/crypto/kdf/service.go`, `internal/crypto/kdf/scrypt.go`, `internal/crypto/kdf/pbkdf2.go`, `internal/crypto/kdf/analyzer.go`, `internal/crypto/kdf/types.go`, `internal/crypto/kdf/interfaces.go`. 🟢

## Responsabilidades

- Manter o contrato operacional descrito para `internal/crypto/kdf/derivar-chave` no legado. 🟢
...
|----|-----------|-----------|-------------------|
| RF-01 | Reimplementar a responsabilidade principal da unit `internal/crypto/kdf/derivar-chave`. | Must | Chamadas equivalentes às funções mapeadas em `internal/crypto/kdf/service.go`, `internal/crypto/kdf/scrypt.go`, `internal/crypto/kdf/pbkdf2.go`, `internal/crypto/kdf/analyzer.go`, `internal/crypto/kdf/types.go`, `internal/crypto/kdf/interfaces.go` produzem resultados compatíveis. 🟢 |
...
```gherkin
Dado uma entrada válida para `internal/crypto/kdf/derivar-chave`
...
Dado que um módulo consumidor depende de `internal/crypto/kdf/derivar-chave`
