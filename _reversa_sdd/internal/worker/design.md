# Módulo internal/worker, Design Técnico
...
A unit `internal/worker` é implementada no legado principalmente em `internal/worker/pool.go`, `internal/worker/stats.go`, `internal/worker/interface.go`. Sua interface é formada por funções, structs e contratos internos consumidos pelos demais módulos do monólito CLI. 🟢
