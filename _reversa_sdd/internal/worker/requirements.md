...
A unit `internal/worker` executa geração concorrente com fan-out de goroutines, primeiro resultado vencedor e métricas. A especificação é baseada nos artefatos Archaeologist/Detective/Architect e nos arquivos principais `internal/worker/pool.go`, `internal/worker/stats.go`, `internal/worker/interface.go`. 🟢

## Responsabilidades

- Manter o contrato operacional descrito para `internal/worker` no legado. 🟢
...
|----|-----------|-----------|-------------------|
| RF-01 | Reimplementar a responsabilidade principal da unit `internal/worker`. | Must | Chamadas equivalentes às funções mapeadas em `internal/worker/pool.go`, `internal/worker/stats.go`, `internal/worker/interface.go` produzem resultados compatíveis. 🟢 |
...
```gherkin
Dado uma entrada válida para `internal/worker`
...
Dado que um módulo consumidor depende de `internal/worker`
